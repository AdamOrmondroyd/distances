...
def alpha_plot(x, mean, sigma, ax, color, dkl=None, linecolor='C0'):
    ...
    if dkl is not None:
        ...
        # build a 1×len(x) RGBA image whose alpha varies horizontally
        rgba = np.zeros((1, x.size, 4))
        rgba[0, :, :3] = mcolors.to_rgb(color)  # constant RGB across the row
        rgba[0, :,  3] = alpha                  # put your dkl-derived alpha here

        # show it stretched over the full x-span, and
        # from ymin to ymax of your band
        im = ax.imshow(rgba,
                       origin='lower',
                       aspect='auto',
                       extent=(x[0], x[-1],
                               np.min(lower), np.max(upper)),
                       interpolation='bilinear',
                       zorder=0)

        # now build one single Path for the under-curve region
        verts = np.vstack([
        ...
        # clip the image to that patch
        im.set_clip_path(patch)

        for _l in [lc, lct, lcb]:
            ax.add_collection(_l)
    else:
        ax.plot(x, mean, color=color, linestyle='--')
        ax.fill_between(x, mean-sigma, mean+sigma, color=color, alpha=0.5)


def alpha_plot_log(x, mean, sigma, ax, color, dkl=None, linecolor='C0'):
    lower = mean - sigma
    upper = mean + sigma
